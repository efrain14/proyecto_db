...
def vincular_salto_enter(widget_actual, widget_siguiente):
    """Profesor: Esta función asocia la tecla Enter para mover el cursor al siguiente campo."""
...
def mostrar_dashboard():
    ...
    # Variable global interna para recordar qué titular estamos editando
    ...
    # Contenedor para organizar con etiquetas ARRIBA de los campos
    ...
    # Fila 0 y 1: Cédula, Nombres, Apellidos
    ...
    # Fila 2 y 3: Fecha Nac, Teléfono, Correo
    ...
    lbl_edad_titular = ctk.CTkLabel(frame_form_reg, text="Edad: -- años", font=("Arial", 12, "bold"), text_color="#1f538d")
    ...
    def disparar_calculo_edad(*args):
        ...
        if len(texto) == 10:
            ...
            else: lbl_edad_titular.configure(text="Edad: Inválida")
    ...
    # Fila 4 y 5: Correo, Dirección, Tipo Contrato
    ...
    # Profesor: Vinculamos el salto automático con la tecla ENTER
    ...
    # Tabla de familiares asociados (Pestaña 1)
    ...
    def guardar_titular():
        ...
        try:
            ...
            conn.commit()
            conn.close()
            messagebox.showinfo("Éxito", "Titular guardado.")
    ...
    # =========================================================================
    # PESTAÑA 2: EDICIÓN COMPLETA (TITULARES Y AFILIADOS)
    # =========================================================================
    ...
    # Frame de edición de datos propios del titular
    ...
    # Lista Excel de afiliados en la pestaña de edición
    ...
    def cargar_datos_edicion():
        ...
        if res:
            ...
            # Cargar los familiares en la tabla de edición
            ...
            messagebox.showinfo("Cargado", f"Datos del titular {res[0]} listos.")
        else:
            messagebox.showerror("Error", "No se encontró ningún registro.")
        conn.close()
    ...
    def agregar_familiar_desde_edicion():
        abrir_modulo_familiares(ventana, cedula_titular_edicion[0], v_ced_fam, v_letras, cargar_datos_edicion)

    # Botones de la Pestaña 2
...
def abrir_modulo_familiares(ventana_padre, cedula_titular, v_ced, v_let, funcion_exito_refrescar):
    ...
    # Etiquetas externas en el pop-up también
